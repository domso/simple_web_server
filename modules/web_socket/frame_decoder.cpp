...
size_t web_server::modules::web_socket::frame_decoder::unpack_data(
    ...
    if (region.size() < 2) {
        ...
        if (byte_len < 126) {
            ...
            data_length = (static_cast<uint64_t>(region.data()[2]) << 56) | 
                        (static_cast<uint64_t>(region.data()[3]) << 48) |
                        (static_cast<uint64_t>(region.data()[4]) << 40) | 
                        (static_cast<uint64_t>(region.data()[5]) << 32) | 
                        (static_cast<uint64_t>(region.data()[6]) << 24) | 
                        (static_cast<uint64_t>(region.data()[7]) << 16) | 
                        (static_cast<uint64_t>(region.data()[8]) << 8)  |
                        static_cast<uint64_t>(region.data()[9]);
            header_length = 10;
        } else {
            return 0;
        }
